...
export const ProductCard = ({ id, title, price, thumbnail, brand, stockQuantity, handleAddRemoveFromWishlist, isWishlistCard, isAdminCard }) => {
    ...
    const wishlistItems = useSelector(selectWishlistItems) || []; // Ensuring it's an array
    const loggedInUser = useSelector(selectLoggedInUser);
    const cartItems = useSelector(selectCartItems) || []; // Ensuring it's an array
    ...
    // Safely checking if product exists in wishlist
    const isProductAlreadyinWishlist = wishlistItems.some((item) => item.product && item.product._id === id);
    // Safely checking if product exists in cart
    const isProductAlreadyInCart = cartItems.some((item) => item.product && item.product._id === id);
